...
export var operators: [Match: RegExp, Meta: string, Precedence: number, LeftRightAssociative: boolean][] = [
  [/\*/, "mul", 12, true],
  [/\//, "div", 12, true],
  [/\+/, "add", 11, true],
  [/%/, "mod", 12, true],
...
export class MathExp extends Expression implements RightDonor {
  ...
  Evaluate(scope: Scope): [stack: VarType[], body: string[]] {
    ...
    if(left[0].length < 1)throw new Error(`Leftside expression does not resolve to any value.`);
    ...
    var metamethod: [ReturnTypes: VarType[], ArgTypes: VarType[], Code: string[]]|null = null;
    for(var i=0; i < bothArgs.length; i++){
      metamethod = bothArgs[i].GetDefinition().GetMetamethod(this.Operator, bothArgs);
      if(metamethod !== null){
        break;
      }
    }
    if(metamethod === null){
      throw new Error(`No method for operator '${this.Operator}' for types ${bothArgs}`);
    }
