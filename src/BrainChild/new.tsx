...
export class New extends Expression {
    ...
    Evaluate(scope: Scope): [stack: VarType[], body: string[]] {
        ...
        if(objectType === undefined)throw new Error(`Cannot find type ${this.Type!.Name}`);
        var classType = objectType.TypeType;
        if(!classType)throw new Error(`Cannot find type type for ${this.Type!.Name}`);
        var targTypeNewMethod = classType.Children["new"];
        if(targTypeNewMethod === undefined)throw new Error(`Cannot find new method for type ${this.Type!.Name}`);
        o.push(`seta ${classType.ClassLabel}`, `adda ${targTypeNewMethod[1]}`, `ptra`, `calla`);
        ...
        var argTypes: VarType[] = [vType];
        for(let i=0; i < this.Arguments.length; i++){
            var arg = this.Arguments[i].Evaluate(scope);
            o.push(...arg[1]);
            argTypes.push(...arg[0]);
        }
        var constructorMetamethod = objectType.GetMetamethod(this.Type!.Name, argTypes);
        if(!constructorMetamethod)throw new Error(`Cannot find constructor new ${argTypes}`);
