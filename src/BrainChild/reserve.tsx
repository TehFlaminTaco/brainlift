...
export class Reserve extends Expression {
    ...
    static Claim(claimer: Claimer): Reserve | null {
        ...
        let r = Reserve.ClaimType(claimer, ret)
             ?? Reserve.ClaimLength(claimer, ret);
        if (r === null) {
            ret.Fail();
            return null;
        }
    ...
    GetSize(scope: Scope): number|null {
        if (this.Length !== null){
            let n = this.Length.Simplify(scope);
            if (n === null){
    ...
    Evaluate(scope: Scope): [stack: VarType[], body: string[]] {
        if (this.Length !== null){
            if (this.ClassMember) return [[VarType.VoidPtr], [this.GetLine(), `apopb`, `apushb`, `incb`, `apushb`]];
        ...
        if (!this.ClassMember){
            ...
            asm.push(`apopb`, `apushb`, `incb`, `bpushb`);
        }
